...
const mockUseSearchParams = vi.fn(() => [{ get: vi.fn() }]);
...
vi.mock('react-router-dom', async (importOriginal) => {
  ...
  return {
    ...
    useNavigate: () => mockNavigate, // Sobrescreve apenas o hook de navegação
...
describe('AuthForm (Integração de Login/Registro)', () => {
  ...
  test('deve autenticar com sucesso e navegar para o dashboard com credenciais corretas', async () => {
    // Renderiza o formulário na aba de Login por padrão
    render(<AuthForm />);
  ...
  test('deve mudar para a aba de Cadastro quando o botão é clicado', () => {
    render(<AuthForm />);

    // 1. Verificar se a aba inicial é Login
    expect(screen.getByRole('heading', { name: /acessar portal/i })).toBeInTheDocument();
    
    // 2. Clicar no botão de "Cadastrar-se"
    fireEvent.click(screen.getByRole('button', { name: /cadastrar-se/i }));

    // 3. Verificar se o conteúdo da aba de Registro foi renderizado
    expect(screen.getByRole('heading', { name: /criar nova conta/i })).toBeInTheDocument();
    expect(screen.getByLabelText(/nome da indústria/i)).toBeInTheDocument(); // Campo específico de registro
  });
});